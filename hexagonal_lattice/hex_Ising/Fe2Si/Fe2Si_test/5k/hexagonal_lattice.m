...
delta_theta =  fix(unifrnd(0,2,1,N))*pi;
delta_phi = unifrnd(0,1,1,N)*2*pi;
...
%%{
T = 5;
Mtest = ones(1,updates)*0;
Etest = ones(1,updates)*0;
%}
...
for ii = 1:updates
    ...
    %%{
        ...
        Mtest(ii) = M_z;
        Etest(ii) = H(delta_theta,delta_phi);
    %}
...
dlmwrite('Mtest.txt', Mtest, 'delimiter', '\t');
dlmwrite('Etest.txt', Etest, 'delimiter', '\t');
